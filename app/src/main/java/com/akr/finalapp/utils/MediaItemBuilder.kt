...
import com.akr.finalapp.data.service.loadArtworkBytesViaCoil
import java.io.File
...
object MediaItemBuilder {
    ...
    private const val EXTERNAL_EXTRA_PREFIX = "com.akr.finalapp.external."
    ...
    fun buildForExternalController(context: Context, song: Song): MediaItem {
        ...
        return com.akr.finalapp.data.diagnostics.PerformanceMetrics.time(
            com.akr.finalapp.data.diagnostics.PerformanceMetrics.Timings.MEDIASESSION_ITEM_BUILD
        ) {
            MediaItem.Builder()
                .setMediaId(song.id)
                .setUri(playbackUri(song))
    ...
    internal fun playbackMimeType(
        ...
        // FIX: If it is a YouTube audio stream, return null to let ExoPlayer auto-sniff container metadata
        if (mimeType == "audio/youtube" || contentUriString.contains("googlevideo.com")) {
            return null
        }
