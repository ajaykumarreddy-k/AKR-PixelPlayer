...
class YoutubeViewModel @Inject constructor(
    ...
    private val _searchFilter = MutableStateFlow(SearchFilterType.ALL)
    ...
    fun updateSearchFilter(filter: SearchFilterType) {
    ...
    fun search() {
        ...
        viewModelScope.launch {
            ...
            val result = youtubeRepository.search(query, filter)
            if (result.isSuccess) {
                val pair = result.getOrNull()
                _searchResults.value = pair?.first ?: emptyList()
                currentContinuation = pair?.second
            } else {
    ...
    fun loadMore() {
        ...
        viewModelScope.launch {
            isLoadingMore = true
            val result = youtubeRepository.searchContinuation(continuation)
            if (result.isSuccess) {
                val pair = result.getOrNull()
                if (pair != null) {
                    val currentList = _searchResults.value.toMutableList()
                    currentList.addAll(pair.first)
                    _searchResults.value = currentList
                    currentContinuation = pair.second
                }
            } else {
                _errorMessage.value = result.exceptionOrNull()?.localizedMessage ?: "Failed to load more"
            }
            isLoadingMore = false
        }
    }
