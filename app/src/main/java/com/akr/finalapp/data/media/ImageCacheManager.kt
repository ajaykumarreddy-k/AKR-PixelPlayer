...
class ImageCacheManager @Inject constructor(
    ...
    fun invalidateCoverArtCaches(vararg uriStrings: String?) {
        ...
        uriStrings.mapNotNull { it?.takeIf(String::isNotBlank) }.forEach { baseUri ->
            if (com.akr.finalapp.utils.LocalArtworkUri.isLocalArtworkUri(baseUri)) {
                com.akr.finalapp.utils.LocalArtworkUri.parseSongId(baseUri)?.let { songId ->
                    com.akr.finalapp.utils.AlbumArtUtils.clearCacheForSong(context, songId)
                }
            }
